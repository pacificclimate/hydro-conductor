...
class Scalar(object):
  ...
  def __set__(self, instance, value):
    try:
      ...
      raise ValueError("Cannot convert '{}' to type {}".format(value,\
      self.type_))
...
class OutfileList(object):
  ...
  def __set__(self, instance, value):
    """Assume assignment will happen in order and just maintain state along the
      way
    """
    try:
...
class Global(metaclass = OrderedMeta):
  ...
  july_tavg_supplied = Boolean()
  temp_th_type = Scalar(str)
  parallel_threads = Scalar(int)
  init_state = Filename()
