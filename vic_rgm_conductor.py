...
# should this be a command line parameter?
...
class OrderedDefaultdict(OrderedDict):
    ...
    def __init__(self, *args, **kwargs):
        if not args:
            ...
            if not (
                    args[0] is None or isinstance(args[0], collections.Callable)):
...
def parse_input_parms():
    ...
    parser.add_argument(
        '--g',
        action="store",
        dest="vic_global_file",
        type=str,
        help='file name and path of the VIC global parameters file')
    parser.add_argument(
        '--rgm-params',
        action="store",
        dest="rgm_params_file",
        type=str,
        help='file name and path of the Regional Glacier Model (RGM) parameters file')
    parser.add_argument(
        '--sdem',
        action="store",
        dest="surf_dem_file",
        type=str,
        help='file name and path of the initial Surface Digital Elevation Model (SDEM) file (GSA format)')
    parser.add_argument(
        '--bdem',
        action="store",
        dest="bed_dem_file",
        type=str,
        help='file name and path of the Bed Digital Elevation Model (BDEM) file (GSA format)')
    parser.add_argument(
        '--pixel-map',
        action="store",
        dest="pixel_cell_map_file",
        type=str,
        help='file name and path of the RGM Pixel to VIC Grid Cell mapping file')
    parser.add_argument(
        '--glacier-mask',
        action="store",
        dest="init_glacier_mask_file",
        type=str,
        help='file name and path of the file containing the initial glacier mask (GSA format)')
    parser.add_argument(
        '--trace-files',
        action="store_true",
        default=False,
        dest="trace_files",
        help='write out persistent GSA format surface DEMs and glacier masks, and 2D mass balance grid files, on each time step for offline inspection')
    parser.add_argument(
        '--bare-soil-root',
        action="store",
        dest="bare_soil_root_parms_file",
        type=str,
        default='NA',
        help='file name and path of one-line text file containing 6 custom root parameters for the bare soil vegetation type / HRU (same format as a vegetation tile line in the vegetation parameters file).  Default: 0.10  1.00  0.10  0.00  0.10  0.00')
    parser.add_argument(
        '--glacier-root',
        action="store",
        dest="glacier_root_parms_file",
        type=str,
        default='NA',
        help='file name and path of one-line text file containing 6 custom root parameters for the glacier vegetation type / HRU (same format as a vegetation tile line in the vegetation parameters file).  Default: 0.10  1.00  0.10  0.00  0.10  0.00')
    parser.add_argument(
        '--band-size',
        action="store",
        dest="band_size",
...
def read_gsa_headers(dem_file):
    ...
    with open(dem_file, 'r') as f:
            # First line
        abbreviation, _ = f.readline().split(None, 1)
        assert (abbreviation == 'DSAA'), 'read_gsa_headers({}): DSAA header on first line of DEM file was not found or is malformed.  DEM file does not conform to ASCII grid format.'.format(
            dem_file)
        # Second line
        num_cols, num_rows, _ = f.readline().split(None, 2)
        ...
        xmin, xmax, _ = f.readline().split(None, 2)
        ymin, ymax, _ = f.readline().split(None, 2)
        ...
        for line in iter(f):
            ...
            if split_line[0] == 'NCOLS':
                ...
                if num_cols_dem and num_rows_dem:
                    pixel_to_cell_map = [
                        [0 for x in range(0, num_cols_dem)] for x in range(0, num_rows_dem)]
                ...
                # NOTE: we might want Markus to recreate this mapping file with
                # zero-based indexing
    ...
    try:
        for row in range(0, num_rows_dem):
            for col in range(0, num_cols_dem):
                ...
                if cell_id != 'NA':
                    # check that the cell_id agrees with what was read from the
                    # veg_parm_file
                    if cell_id not in cell_ids:
                        print ('mass_balances_to_rgm_grid: Cell ID {} was not found in the list of VIC cell IDs read from the vegetation parameters file. Exiting.\n').format(
                            cell_id)
                        sys.exit(0)
                    mass_balance_grid[row][col] = gmb_polys[cell_id][
                        0] + median_elev * (gmb_polys[cell_id][1] + median_elev * gmb_polys[cell_id][2])
    except:
        print ('mass_balances_to_rgm_grid: Error while processing pixel {} (row {} column {})').format(
            pixel,
            row,
            col)
    ...
    with open(global_parm_file, 'r') as f:
        for line in iter(f):
            ...
            if not line.isspace() and line[0] is not '#':
                ...
                # special case because there are multiple occurrences, not
                # consecutive
                if parm_name == 'OUTFILE':
                    n_outfile_lines += 1
                    parm_name = 'OUTFILE_' + str(n_outfile_lines)
                # special case because multiple OUTVAR lines follow each
                # OUTFILE line
                elif parm_name == 'OUTVAR':
                    parm_name = 'OUTVAR_' + str(n_outfile_lines)
                try:
                    # if we've already read one or more entries of this
                    # parm_name
                    if global_parms[parm_name]:
...
#					print ('global_parms[{}]: {}').format(parm_name,global_parms[parm_name])
                ...
                # global parameters file, to be used for all iterations after
                # the first year
                # OUTPUT_FORCE should always immediately precede INIT_STATE in
                # the global file
                if parm_name == 'OUTPUT_FORCE':
...
# TODO: need to update this to support VIC wind-up from STARTYEAR to
# GLAC_ACCUM_START_YEAR
    global_parms['STARTYEAR'] = [[str(year)]]
    global_parms['ENDYEAR'] = [[str(year)]]
    global_parms['STATEYEAR'] = [[str(year)]]
    # All iterations after the first / wind-up period have modified state,
    # vegetation parms, and snow band parms
    if init_state:
        # set/create INIT_STATE parm with most current state_file (does not
        # exist in the first read-in of global_parms)
        init_state_file = state_filename_prefix + "_" + \
            str(year - 1) + str(global_parms['STATEMONTH']
                                [0][0]) + str(global_parms['STATEDAY'][0][0])
    ...
    with open(temp_gpf, 'w') as f:
        ...
        try:
            for parm in global_parms:
                ...
                if parm == 'INIT_STATE' and len(
                        global_parms['INIT_STATE']) == 0:
                    pass
            ...
            print (
                'write_global_parms_file: Error while writing parameter {} to file. Exiting.\n').format(parm)
            sys.exit(0)
    return temp_gpf
        ...
        for band in veg_parms[cell]:
            ...
            for line_idx, line in enumerate(veg_parms[cell][band]):
                ...
                if line[0] == GLACIER_ID:
                    glacier_exists = True
                    residual_area_fracs[cell][band] = snb_parms[cell][0][
                        int(band)] - float(veg_parms[cell][band][line_idx][1])
                    print ('Glacier exists in this band. residual_area_fracs[{}][{}] = {} - {} = {}').format(
                        cell,
                        band,
                        snb_parms[cell][0][
                            int(band)],
                        float(
                            veg_parms[cell][band][line_idx][1]),
                        residual_area_fracs[cell][band])
            ...
            if not glacier_exists:
                ...
                print ('No glacier in this band.  residual_area_fracs[{}][{}] = {}').format(
                    cell,
                    band,
                    residual_area_fracs[cell][band])
...
def update_band_areas():
    ...
    for row in range(0, num_rows_dem):
        for col in range(0, num_cols_dem):
            # get the VIC cell this pixel belongs to
            cell = pixel_to_cell_map[row][col][0]
            if cell != 'NA':
                #				print ('row: {}, col: {}').format(row, col)
                #				print ('cell: {}').format(cell)
                # Use the RGM DEM output to update the pixel median elevation
                # in the pixel_to_cell_map
...
#		raw_input('--------------- row processed. Hit enter --------------')
    print ('Area fractions of elevation bands within VIC cells: {}').format(
        band_areas)
    print ('Area fraction of glacier within VIC cells, per elevation band: {}').format(
        glacier_areas)
    print ('\n')
    for cell in cell_ids:
        print ('cell: {}').format(cell)
        ...
        for band_idx, band in enumerate(band_map[cell]):
            print ('band: {}, band_idx: {}').format(band, band_idx)
            print ('band_areas[{}][{}] = {}').format(
                cell,
                band_idx,
                band_areas[cell][band_idx])
            print ('glacier_areas[{}][{}] = {}').format(
                cell,
                band_idx,
                glacier_areas[cell][band_idx])
            # This will be used to update the Snow Band File
            area_frac_bands[cell][band_idx] = float(
                band_areas[cell][band_idx]) / cell_areas[cell]
            ...
            area_frac_glacier[cell][band_idx] = float(
                glacier_areas[cell][band_idx]) / cell_areas[cell]
            if area_frac_glacier[cell][band_idx] < 0:
                print ('update_band_areas(): Error: Calculated a negative glacier area fraction for cell {}, band {}. Exiting.\n').format(
                    ...
                    band)
                sys.exit(0)
            print ('area_frac_bands[{}][{}] = {}').format(
                cell,
                band_idx,
                area_frac_bands[cell][band_idx])
            print ('area_frac_glacier[{}][{}] = {}').format(
                cell,
                band_idx,
                area_frac_glacier[cell][band_idx])
            print ('\n')
...
def update_veg_parms():
    ...
    for cell in cell_ids:
        print ('cell: {}').format(cell)
        for band in veg_parms[cell]:
            # If we (still) have anything in this elevation band
            if area_frac_bands[cell][int(band)] > 0:
                new_residual_area_frac = area_frac_bands[cell][
                    int(band)] - area_frac_glacier[cell][int(band)]
                delta_residual = residual_area_fracs[
                    cell][band] - new_residual_area_frac
                # identify and temporarily store vegetation types (HRUs)
                # currently existing within this band
                veg_types = []
                # temporary boolean to identify if bare soil vegetation type
                # (HRU) currently exists within this band
                bare_soil_exists = False
                # temporary boolean to identify if glacier vegetation type
                # (HRU) currently exists within this band
                glacier_exists = False
                # sum of band vegetation tile (HRU) area fractions in the last
                # iteration
                sum_previous_band_area_fracs = 0
                # glacier portion of this band has SHRUNK; update its area
                # fraction and increase the bare soil component accordingly
                if delta_residual < 0:
                    print ('\nGlacier portion of band {} has SHRUNK (delta_residual = {})').format(
                        band,
                        delta_residual)
                    for line_idx, line in enumerate(veg_parms[cell][band]):
                        print ('Current line in band {}:  {}').format(
                            band,
                            line)
                        veg_types.append(int(line[0]))
                        print ('Existing vegetation type identified in this elevation band: {}').format(
                            veg_types[-1])
                        if veg_types[-1] == int(BARE_SOIL_ID):
                            bare_soil_exists = True
                            veg_parms[cell][band][line_idx][1] = str(
                                float(
                                    veg_parms[cell][band][line_idx][1]) +
                                abs(delta_residual))
                            print ('Bare soil already exists in this band.  New area fraction: veg_parms[{}][{}][{}][1] = {}').format(
                                cell,
                                band,
                                line_idx,
                                veg_parms[cell][band][line_idx][1])
                            ...
                            veg_parms[cell][band][line_idx][1] = str(
                                area_frac_glacier[cell][
                                    int(band)])
                            print ('Updated glacier area fraction: veg_parms[{}][{}][{}][1] = {}').format(
                                cell,
                                band,
                                line_idx,
                                veg_parms[cell][band][line_idx][1])
                    # insert new line for bare soil vegetation (HRU) type in
                    # the correct numerical position within this band
                    if not bare_soil_exists:
                        ...
                        print ('Bare soil did NOT already exist in this band. Inserted line: veg_parms[{}][{}][{}] = {}').format(
                            cell,
                            band,
                            position,
                            veg_parms[cell][band][position])
                # glacier portion of this band has GROWN; decrease fraction of
                # non-glacier HRUs by a total of delta_residual
                elif delta_residual > 0:
                    print ('\nGlacier portion of band {} has GROWN (delta_residual = {})').format(
                        band,
                        delta_residual)
                    for line_idx, line in enumerate(veg_parms[cell][band]):
                        sum_previous_band_area_fracs += float(line[1])
                    print ('sum_previous_band_area_fracs = {}').format(
                        sum_previous_band_area_fracs)
                    for line_idx, line in enumerate(veg_parms[cell][band]):
                        print ('Current line in band {}:  {}').format(
                            band,
                            line)
                        # keep a record of vegetation types (HRUs) currently
                        # existing within this band
                        veg_types.append(int(line[0]))
                        print ('Existing vegetation type identified in this elevation band: {}').format(
                            veg_types[-1])
                        # set glacier tile area fraction
                        if veg_types[-1] == int(GLACIER_ID):
                            glacier_exists = True
                            veg_parms[cell][band][line_idx][1] = str(
                                area_frac_glacier[cell][
                                    int(band)])
                            print ('Glacier already exists in this band.  New glacier area fraction: veg_parms[{}][{}][{}][1] = {}').format(
                                cell,
                                band,
                                line_idx,
                                veg_parms[cell][band][line_idx][1])
                        else:
                            # Get the change in area fraction for this
                            # vegetation type (HRU) based on its previous share
                            # of the residuals in this elevation band
                            print ('Existing area fraction for this vegetation type: {}').format(
                                veg_parms[cell][band][line_idx][1])
                            delta_veg_area_frac = delta_residual * \
                                (float(
                                    veg_parms[cell][band][line_idx][1]) / sum_previous_band_area_fracs)
                            print ('Calculated delta_veg_area_frac = {}').format(
                                delta_veg_area_frac)
                            new_veg_area_frac = float(
                                veg_parms[cell][band][line_idx][1]) - delta_veg_area_frac
                            if new_veg_area_frac >= 0:
                                veg_parms[cell][band][line_idx][
                                    1] = str(new_veg_area_frac)
                                print ('Reduced area fraction of vegetation type {} by delta_veg_area_frac.  New area fraction: veg_parms[{}][{}][{}][1] = {}').format(
                                    veg_types[-1], cell, band, line_idx, veg_parms[cell][band][line_idx][1])
                            # the existing vegetation tile (HRU) was overtaken
                            # by glacier
                            else:
                                ...
                                print ('update_veg_parms(): Error: Calculated a negative area fraction ({}) for vegetation type {} in cell {}, band {}. Exiting.\n').format(
                                    new_veg_area_frac, veg_types[-1], cell, band)
                                sys.exit(0)
                    # insert new line for glacier vegetation (HRU) type in the
                    # correct numerical position within this band
                    if not glacier_exists:
                        ...
                        new_line.append(
                            str(area_frac_glacier[cell][int(band)]))
                        ...
                        veg_parms[cell][band].insert(position, new_line)
                        print ('Glacier did NOT already exist in this band. Inserted line with new glacier area fraction: veg_parms[{}][{}][{}] = {}').format(
                            cell,
                            band,
                            position,
                            veg_parms[cell][band][position])
            # We have nothing left in this elevation band.  Set all fractional
            # areas for all vegetation tiles (HRUs) in this band to zero
            else:
                print (
                    '\nNothing left in elevation band {}.  Setting all fractional areas to zero.').format(band)
                for line_idx, line in enumerate(veg_parms[cell][band]):
                    veg_parms[cell][band][line_idx][1] = 0
            # Set residual area fractions to the new calculated values for the
            # next iteration
...
def write_veg_parms_file():
    ...
    print ('writing new temporary vegetation parameter file {}...').format(
        temp_vpf)
        ...
        for line in iter(f):
            ...
            if num_columns != 3 * num_snow_bands + 1:
                print ('get_snb_parms(): Error: Number of columns ({}) in snow band file {} is incorrect for the given number of snow bands ({}) given in the global parameter file (should be 3 * num_snow_bands + 1). Exiting.\n').format(
                    ...
                    num_snow_bands)
                sys.exit(0)
            snb_parms[cell_id] = [[float(x) for x in split_line[1: num_snow_bands +
                                                                1]], [int(x) for x in split_line[num_snow_bands +
                                                                                                 1: 2 *
                                                                                                 num_snow_bands +
                                                                                                 1]], [float(x) for x in split_line[2 *
                                                                                                                                    num_snow_bands +
                                                                                                                                    1: 3 *
                                                                                                                                    num_snow_bands +
                                                                                                                                    1]]]
...
def create_band_map(snb_parms, band_size):
    ...
    # TODO: create command line parameter to allow for extra bands to be
    # specified as padding above/below existing ones, to allow for glacier
    # growth(/slide?)
    band_map = {}
    for cell in cell_ids:
...
def update_glacier_mask(sdem, bdem):
    ...
    if np.any(diffs < 0):
        print (
            'update_glacier_mask: Error: subtraction of Bed DEM from output Surface DEM of RGM produced one or more negative values.  Exiting.\n')
        sys.exit(0)
...
if __name__ == '__main__':
    ...
    # Get entire time range of coupled VIC-RGM run from the initial VIC global
    # file
    start_year = int(global_parms['STARTYEAR'][0][0])
    end_year = int(global_parms['ENDYEAR'][0][0])
    # Initial VIC output state filename prefix is determined by STATENAME in
    # the global file
    ...
    # Get VIC vegetation parameters and grid cell IDs from initial Vegetation
    # Parameter File
    ...
    # Calculate the initial residual (i.e. non-glacier) area fractions for all
    # bands in all cells
    residual_area_fracs = init_residual_area_fracs()

    # Open and read VIC-grid-to-RGM-pixel mapping file
    # pixel_to_cell_map is a list of dimensions num_rows_dem x num_cols_dem,
    # each element containing a VIC grid cell ID
    pixel_to_cell_map, num_rows_dem, num_cols_dem, cell_areas = get_rgm_pixel_mapping(
        pixel_cell_map_file)

    # Check header validity, verify number of columns & rows agree with what's
    # stated in the pixel_to_cell_map_file, and get DEM xmin, xmax, ymin, ymax
    # metadata
    dem_xmin, dem_xmax, dem_ymin, dem_ymax = read_gsa_headers(bed_dem_file)
    # Read the provided Bed Digital Elevation Map (BDEM) file into a 2D
    # bed_dem array
    bed_dem = np.loadtxt(bed_dem_file, skiprows=5)

    # The RGM will always output a DEM file of the same name (if running RGM
    ...
    for year in range(start_year, end_year):
        ...
        # Overwrite VEGPARAM parameter with temp_vpf, and SNOW_BAND with
        # temp_snb
        state_file = state_filename_prefix + "_" + \
            str(year) + str(global_parms['STATEMONTH'][0]
                            [0]) + str(global_parms['STATEDAY'][0][0])
        ...
        # 2. Run VIC for a year.  This will save VIC model state at the end of
        # the year, along with a Glacier Mass Balance (GMB) polynomial for each
        # cell
        subprocess.check_call(
            [vic_full_path, "-g", temp_gpf], shell=False, stderr=subprocess.STDOUT)

        # 3. Open VIC NetCDF state file and get the most recent GMB polynomial
        # for each grid cell being modeled
        ...
        # 4. Translate mass balances using grid cell GMB polynomials and
        # current veg_parm_file into a 2D RGM mass balance grid (MBG)
        mass_balance_grid = mass_balances_to_rgm_grid(
            gmb_polys,
            pixel_to_cell_map)
        # write Mass Balance Grid to ASCII file to direct the RGM to use as
        # input
        ...
        subprocess.check_call([rgm_full_path,
                               "-p",
                               rgm_params_file,
                               "-b",
                               bed_dem_file,
                               "-d",
                               rgm_surf_dem_in_file,
                               "-m",
                               mbg_file,
                               "-o",
                               temp_files_path,
        ...
        temp_surf_dem_file = temp_files_path + \
            'rgm_surf_dem_out_' + str(year) + '.gsa'
        ...
        if output_trace_files:
            glacier_mask_file = temp_files_path + \
                'glacier_mask_' + str(year) + '.gsa'
            write_grid_to_gsa_file(glacier_mask, glacier_mask_file)

        # 8. Update areas of each elevation band in each VIC grid cell, and
        # calculate area fractions
        area_frac_bands, area_frac_glacier = update_band_areas()

        # 9. Update vegetation parameters and write to new temporary file
        # temp_vpf
        update_veg_parms()
        temp_vpf = write_veg_parms_file()

        # 10. Update snow band parameters and write to new temporary file
        # temp_snb
        update_snb_parms()
